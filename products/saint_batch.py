...
from utils import saint_graph_aug, set_seeds, permute_edges
...
parser.add_argument('--load_CL', type=int, default=10000)
...
def train(model, loader, optimizer, device, epoch, args):
    ...
    for data in loader:
        ...
        test = y_pre[data.test_mask]

        y = data.y.squeeze(1)[data.train_mask]
        y_test = data.y.squeeze(1)[data.test_mask]

        loss_train = F.nll_loss(out, y)
        loss_train.backward()
        optimizer.step()

        loss_test = F.nll_loss(test, y_test)
        total_loss += float(loss_train)
        total_losst += float(loss_test)
    loss = total_loss / len(loader)
    with open('/root/OGB/products/train.txt', 'a', encoding='utf-8') as f:
        f.write("%.6f" % loss)
        f.write(',')

    loss_t =total_losst / len(loader)
    with open('/root/OGB/products/test.txt', 'a', encoding='utf-8') as f:
        f.write("%.6f" % loss_t)
        f.write(',')
