...
import numpy as np
import sys
from utils import set_seeds, ns_graph_aug, ns_mask_nodes
...
def train_products(model, clean, y, adjs, adja, args, optimizer, device, criterion, train_idx=None) :
    ...
    if train_idx is not None:
        ...
        model_forward2 = lambda x: model(x, adjs)
    optimizer.zero_grad()


    out, x1, g1 = model_forward1(clean)
    _, x2, g2 = model_forward2(adja)
...
def train(epoch):
    ...
    for batch_size, n_id, adjs in train_loader:
        ...
        data = deepcopy(clean)
        data = data.cpu()
        data_aug = ns_mask_nodes(data, args.rate)
        data_aug = data_aug.to(device)

        loss, out = train_products(model, clean, y[n_id[:batch_size]], adjs, data_aug, args, optimizer, device,
                                          F.nll_loss)
