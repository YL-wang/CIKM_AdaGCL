...
class SAGE(torch.nn.Module):
    ...
    #     return self.fc2(z)
    #
    # def cl_lossaug(self, z1, z2, neg_mask):
    #     h1 = self.projection(z1)
    #     h2 = self.projection(z2)
    #     h1 = F.normalize(h1)
    #     h2 = F.normalize(h2)
    #
    #     loss1 = self.negsam_loss(h1, h2, neg_mask)
    #     loss2 = self.negsam_loss(h2, h1, neg_mask)
    #     ret = (loss1 + loss2) / 2
    #
    #     ret = ret.mean()
    #
...
def train_products(model, clean, y, adjs, adja, args, optimizer, device, criterion, train_idx=None) :
    ...
    out, x1 = model_forward1(clean)
    # _, x2 = model_forward2(clean)
    # label = y.contiguous().view(-1,1)

    # mask = torch.eq(label, label.T).float().to(device)
    # neg_mask = 1 - mask

    # loss_cl = model.cl_lossaug(x1, x2, neg_mask)
...
def permute_edges(edge):
    ...
    permute_num2 = int(edge_num2 * 1 / 10)
    permute_num3 = int(edge_num3 * 1 / 15)
...
for run in range(args.runs):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch >20  and epoch % args.test_freq == 0 or epoch == args.epochs:
            result = test()
            tra, val, tst = result
