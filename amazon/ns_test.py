...
import argparse
parser = argparse.ArgumentParser(description='Amazon (SAGE)')
...
def permute_edges(edge):
    _, edge_num1 = edge[0].edge_index.shape #5
    _, edge_num2 = edge[1].edge_index.shape #10
    _, edge_num3 = edge[2].edge_index.shape #15
...
def train(epoch):
    ...
    for batch_size, n_id, adjs in train_loader:
        ...
        #adj_original = deepcopy(adjs)
        adja = permute_edges(adjs)
        clean = x[n_id]

        loss, out = train_products(model, clean, y[n_id[:batch_size]], adja, adja, args, optimizer, device,
                                          criterion)
...
def test(model, split_idx, x, y, subgraph_loader, device):
    ...
    train_rocauc = eval_rocauc(y_true[split_idx["train"]], y_pred[split_idx["train"]])
    val_rocauc = eval_rocauc(y_true[split_idx["valid"]], y_pred[split_idx["valid"]])
    test_rocauc = eval_rocauc(y_true[split_idx["test"]], y_pred[split_idx["test"]])

    return train_rocauc, val_rocauc, test_rocauc
...
for run in range(args.runs):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch >55  and epoch % args.test_freq == 0 or epoch == args.epochs:
