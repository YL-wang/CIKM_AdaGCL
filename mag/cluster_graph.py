...
from utils import permute_edges, set_seeds, cluster_graph_aug
...
cluster_data = ClusterData(homo_data, num_parts=args.num_partitions, recursive=True,
...
class RGCN(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index, edge_type, node_type,
        ...
        return x.log_softmax(dim=-1), out, g
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        view1, x_aug = cluster_graph_aug(x_aug, data_aug, args.rate, cluster, device)
        ...
        data = data.to(device)
        optimizer.zero_grad()
        _, x1, g1 = model(x_aug, view1.edge_index, view1.edge_attr, view1.node_type,
                    view1.local_node_idx, cluster)
        out, x2, g2 = model(x_dict, data.edge_index, data.edge_attr, data.node_type,
                    data.local_node_idx, cluster)
